...
--> 에전 SQL 방식 허용 (계정명을 간단히 작성 가능)

-- [작성법]
-- CREATE USER 사용자명 INDENTIFIED BY 비밀번호;
CREATE USER hjh_sample IDENTIFIED BY 1234;
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한, 권한, ... TO 사용자 명;
GRANT CREATE SESSION TO hjh_sample;
...
GRANT CREATE TABLE TO hjh_sample;

ALTER USER hjh_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;

-- 7. (hjh_sample) 테이블 다시 생성
CREATE TABLE TB_TEST(
	PK_COL NUMBER PRIMARY KEY,
	CONTENT VARCHAR2(100)
);
...
--(SYS) hjh_sample 계정에 CONNECT, RESOURCE ROLE 부여
GRANT CONNECT, RESOURCE TO hjh_sample;

--------------------------------------------------------------------------------------

-- /* 객체 권한 */

-- kh / hjh_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. (hjh_sample) kh 계정의 EMPLOYEE 테이블 조회
SELECT * FROM EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근 권한이 없어서 조회 불가

-- 2. (kh) hjh_sample 계정에 EMPLOYEE 테이블 조회 권한 부여
-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명
GRANT SELECT ON EMPLOYEE TO hjh_sample;

-- 3. (hjh_sample) 다시 kh 계정의 EMPLOYEE 조회
SELECT * FROM kh.EMPLOYEE; --> 조회 가능
SELECT * FROM kh.DEPARTMENT; --> 조회 안됨
--> 위에서 kh가 EMPLOYEE 조회하는 권한만 주었기 때문

-- 4. (kh) hjh_sample 계정에 부여했던 EMPLOYEE 테이블 조회 권한 회수(REVOKE)

-- [권한 회수 작성법]
-- REVOKE 객체권한 ON 객체명 FROM 사용자명;

REVOKE SELECT ON EMPLOYEE FROM hjh_sample;

-- 5. (hjj_sample) EMPLOYEE 조회해보기
SELECT * FROM EMPLOYEE; --> 조회 안된다.
