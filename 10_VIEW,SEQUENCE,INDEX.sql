...
--> 계정명이 언급되는 상황에서는 이 구문을 필수적으로 진행
...
VALUES ('D0','L2');
-- 가상 테이블인 VIEW에서도 INSERT 수행 가능
-- D0 L2

SELECT * FROM DEPT_COPY2;
-- D0 NULL L2
--> VIEW에 INSERT를 수행했지만 VIEW 생성 시 사용한 원본 테이블에 값이 함께 INSERT됨을 확인
-- 사용되지 않은 컬럼(DEPT_TITLE)에는 NULL이 들어감
--> NULL은 DB의 무결성을 약하게 만드는 주요 원인이므로 가능하면 의도되지 않은 NULL의 존재는 주의해야한다
...
-- VIEW에서의 DML(INSERT/DELETE/UPDATE)을 막기 위해 사용
...
--> 시퀀스가 생성 되자마자 CURRVAL 호출할 경우 오류 발생
-- CURRVAL의 정확한 의미는 가장 최근에 호출된 NEXTVAL의 값을 반환함을 뜻함
...
-- NEXTVAL을 호출할 때마다 INCREMENT BY에 작성된 수 만큼 증가하는지 확인
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;
-- 처음: 100
-- 2회: 105
-- 3회: 110
...
-- 컬럼값을 통해서 해당행의 ROWID를 빠르게 찾아낼 수 있다.
...
-- TB_TABLE_TEST 테이블에 샘플데이터 100만개 삽입(PL/SQL 사용)
-- * PL/SQL
-- 오라클 데이터베이스에서 사용하는 절차적 확장 언어
-- 변수, 조건문(IF/CASE), 반복문(LOOP,WHILE) 사용 가능

BEGIN
	FOR I IN 1..1000000 -- I 는 1 부터 1,000,000까지 반복
	LOOP
		INSERT INTO TB_IDX_TEST
		VALUES(I, 'TEST' || I);
	END LOOP;
	COMMIT; -- LOOP의 모든 삽입을 COMMIT
END;
...
WHERE TEST_ID = 'TEST500000'; -- 0.015s
